...
stat_mon=$(date -d "$startDate+0days" +%Y%m)
 sql="
    truncate table dw_hr.fct_rt_dc_shop_sku_vender_day_ti;
		...
		select
		    ...
		    select  toYear(xdt.deliver_date) as stat_year,
		        ...
		        '' as venderid,
		        case when xdt.logistics in ('F', 'T') then 2
		                when xdt.logistics in ('G') THEN 3
		    ...
		    select  toYear(xdt.deliver_date) as stat_year,
		        ...
		        category_id as categoryid_new,
		        venderid,
		        case when xdt.logistics in ('1直流') then 2
		                when xdt.logistics in ('0存储',  '3调整') THEN 3
		            ELSE 0 END logisticsid,
		        1 as buntype,
		        3 as datasource,
   ...
   select frdssvd.stat_year,
          ...
          frdssvd.category_id ,
          goodsid,
          cityHash64(concat(toString(out_buid), toString(goodsid))) as hash_goodsid,
          logistics,
          buntype,
          dctype,
          shopformid,
